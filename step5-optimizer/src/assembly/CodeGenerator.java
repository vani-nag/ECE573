...
*/
		...
		public Map<Integer,LinkedList<String>> liveness(InstructionList isl)
		{
			...
			/*Updates
			1.Removed addi. used only for sp
			2. added check condition to prevent duplicates in liveset
			*/
			...
			while(i >= 0 )
			{
				...
				if(temp.getOC() != null)
				{
					switch(temp.getOC())
					{
						...
						/*case ADDI: liveSet.remove(temp.getDest());if(temp.getsrc1().charAt(0) == '$' && !liveSet.contains(temp.getsrc1()))liveSet.add(temp.getsrc1());break;*/
						...
						case FLT:
	liveSet.remove(temp.getDest());if(temp.getsrc1().charAt(0) == '$' && !liveSet.contains(temp.getsrc1()))liveSet.add(temp.getsrc1());if(temp.getsrc2().charAt(0) == '$' && !liveSet.contains(temp.getsrc2())) liveSet.add(temp.getsrc2()); break;
						case FLE:
	liveSet.remove(temp.getDest());if(temp.getsrc1().charAt(0) == '$' && !liveSet.contains(temp.getsrc1()))liveSet.add(temp.getsrc1());if(temp.getsrc2().charAt(0) == '$' && !liveSet.contains(temp.getsrc2())) liveSet.add(temp.getsrc2()); break;
						case FEQ:
	liveSet.remove(temp.getDest());if(temp.getsrc1().charAt(0) == '$' && !liveSet.contains(temp.getsrc1()))liveSet.add(temp.getsrc1());if(temp.getsrc2().charAt(0) == '$' && !liveSet.contains(temp.getsrc2())) liveSet.add(temp.getsrc2()); break;
	...
	//}//}= for class
		...
		{
			/*
			Updated with src1Reg for int. Float?
			Change == to equals?
			*/
			int ret,j;
			...
			if(intOrFloat == 0)
			{
				...
				{
					System.out.println("Allocate:intRegList "+intRegList.get(j));
					
					//Find non-dirty register
					if(this.dirtyIntRegList.get(j) == 0 && intRegList.get(j) != "no" && intRegList.get(j) != "address" && intRegList.get(j) != this.src1Reg)
					{
						ret = j;
				...
				for(j = 0; j < this.numReg; j++)
				{
					...
					{
						System.out.println("Came here Allocate: this.src1Reg "+String.valueOf(!(intRegList.get(j).equals(this.src1Reg))));
						ret = j;
						free(ret, intOrFloat, liveOut);
						this.intRegList.set(ret, src);
						if(!(this.usedIntRegList.contains(ret)))
			...
			else
			{
				...
				{
					//Find non-dirty register
					if(this.dirtyFloatRegList.get(j) == 0 && floatRegList.get(j) != this.src1Reg)
					{
						ret = j;
						free(ret, intOrFloat, liveOut);
						this.floatRegList.set(ret, src);
				...
				for(j = 0; j < this.numReg; j++)
				{
					if(floatRegList.get(j) != this.src1Reg)
					{
						ret = j;
						free(ret, intOrFloat, liveOut);
						this.floatRegList.set(ret, src);
		...
		public void free(int regNo, int intOrFloat, LinkedList<String> liveSet)
		{
			...
			if(intOrFloat == 0)
			{
				System.out.println("freeing: "+String.valueOf(regNo));
			...
			else
			{
				...
				//if(dirtyFloatRegList.get(regNo) == 1)
				{
					...
					{
						this.code.add(new Fsw("f"+String.valueOf(regNo), "fp", src.substring(2)));
					}
					else if(src.charAt(0) == '$' && src.charAt(1) == 't')
					{
						//HERE
						...
						this.code.add(new Fsw("f"+String.valueOf(regNo), "fp", ste.addressToString()));
					}
